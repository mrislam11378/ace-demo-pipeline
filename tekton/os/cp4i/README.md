...
```
oc --namespace openshift-image-registry port-forward --address 0.0.0.0 svc/image-registry 5000:5000
```
...
```bash
oc create secret generic jdbc-secret -n cp4i --from-literal=USERID='USERNAME' --from-literal=PASSWORD='PASSWORD' --from-literal=databaseName='BLUDB' --from-literal=serverName='19af6446-6171-4641-8aba-9dcff8e1b6ff.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud' --from-literal=portNumber='30699'
```
...
```bash
oc create secret -n cp4i docker-registry regcred --docker-server=image-registry.openshift-image-registry.svc.cluster.local:5000 --docker-username=kubeadmin --docker-password=$(oc whoami -t)
```
...
```bash
oc apply -f tekton/os/cp4i/cp4i-scc.yaml
oc apply -f tekton/os/cp4i/service-account-cp4i.yaml
...
```bash
oc apply -f tekton/os/cp4i/12-ibmint-cp4i-build-task.yaml
oc apply -f tekton/os/cp4i/13-component-test-in-cp4i-task.yaml
oc apply -f tekton/os/cp4i/22-deploy-to-cp4i-task.yaml
oc apply -f tekton/os/cp4i/cp4i-pipeline.yaml
```

and to run the pipeline

```bash
oc create -f tekton/os/cp4i/cp4i-pipeline-run.yaml
